...
if(isset($_POST['song'])) {
  ...
  if(strlen($comments) < 2) {
  	$error_message .= 'Text je moc kratky! <br />'; //'The Comments you entered do not appear to be valid.<br />';
  }*/
  if(strlen($odKoho) < 2) {
  	$odKoho .= 'Nezadáno <br />'; //'The Comments you entered do not appear to be valid.<br />';
  }
  if(strlen($proKoho) < 2) {
  	$proKoho .= 'Nezadáno <br />'; //'The Comments you entered do not appear to be valid.<br />';
  }
  if(strlen($vzkaz) < 5) {
  	header("Location: $shortMsg");
    //$error_message .= 'Vzkaz je moc krátký! <br />'; //'The Comments you entered do not appear to be valid.<br />';
?>
<script>location.replace('<?php echo $shortMsg;?>')</script>
<?php
  exit;
}
 if(strlen($song) < 5) {
  //$error_message .= 'Jméno písničky je moc krátké! <br />'; //'The Comments you entered do not appear to be valid.<br />';
  header("Location: $shortSongName");
?>
<script>location.replace('<?php echo $shortSongName;?>')</script>
<?php
  exit;
}
  /*
...
/*	echo iconv(mb_detect_encoding($odKoho, mb_detect_order(), true), "UTF-8", $odKoho);
	echo iconv(mb_detect_encoding($proKoho, mb_detect_order(), true), "UTF-8", $proKoho);
	...
	$email_message .= "Od: ".clean_string($odKoho)."\r\n";
	$email_message .= "Pro: ".clean_string($proKoho)."\r\n";
	$email_message .= "Song: ".clean_string($song)."\r\n";
	$email_message .= "Vzkaz: ".clean_string($vzkaz)."\r\n";
